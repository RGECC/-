Page({
  ...
  getlist(zhuangtai){
    wx.cloud.callFunction({
      ...
      data:{
        type:'get',                 //请求获取订单
        zhuangtai:zhuangtai               //订单的状态：-1订单取消，0已下单待制作，1已上菜，2订单已完成
      }
    }).then(res=>{
      console.log('请求到的数据',res)
      this.setData({
        list: res.result.data          //将从数据库中获取的订单列表数据 res.result.data 更新到页面的 list 变量中
      })                                //这样就可以显示所有用户下单的订单了
    })
  },
  //制作完成
  zhizuowancheng(e){
    ...
    wx.cloud.callFunction({
      ...
      data:{
        type:'update',                 //更新订单
        id:id,                    //订单号
        zhuangtai:1               //订单的状态：-1订单取消，0已下单待制作，1已上菜，2订单已完成
      }
    }).then(res=>{
